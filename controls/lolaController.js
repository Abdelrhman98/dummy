...
module.exports = function (app) {
                ...
                dbclient.query("SELECT * FROM news LIMIT 5", function (err, rows) {
                    if (err) {
                        ...
                        dbclient.query("SELECT * FROM news  ORDER BY id ASC", function (err, rows) {
                            ...
                            dbClient.end();
                            globalNews.news.reverse();
                            res.render('lola', { news: globalNews.news });
                        });
                    }
                });
            });
    ...
    app.get("/selectlolaphoto", function (req, res) {
        globalNews.news = [];
        ...
        dbclient.query("SELECT * FROM news LIMIT 5", function (err, rows) {
            if (err) {
                ...
                dbclient.query("SELECT * FROM news   ORDER BY id ASC", function (err, rows) {
                    ...
                    dbClient.end();
                    globalNews.news.reverse();
                    res.render('editnobtgya', { news: globalNews.news, nobtgyaName: "", type: "submitlolaphoto" });
                });
            }
        });
    });
    app.post("/submitlolaphoto", function (req, res) {
        var dbClient = db.dbOpenConnection();
        var imgName = "lola_" + Date.now();
        dbclient.connect(function (err, results) {
            if (err) {
                console.log("open connection error", err);
            }
        });
        dbclient.query("INSERT INTO photos (name,type) VALUES ('" + imgName + "','lola')", function (err, results, fields) {
            if (err) {
